...
app.post("/register", checkJoinCode, function(req,res)
{
  ...
  else
  {
    ...
    p1.then(function(_class){
      User.register({username: req.body.username, fname:req.body.fname, lname:req.body.lname, dob:req.body.dob, class:_class, tasks:list_of_tasks}, req.body.password, function(err, user)
      {
        if(err){
          console.log(err.name);
          error = "Your email is already registered! Try using another email."
          res.render('register', {
            'err': error,
          });
        }
...
app.get("/admin/getstudent/:studentid", async function(req, res) {
  ...
  if(req.isAuthenticated()){
    ...
    res.render("adminTasklist",{studentid:req.params.studentid,tasklist:tasklist});
...
app.get("/admin/getstudent/:studentid/:taskid", async function(req, res) {
  ...
  {
    try{
      const sentencelist = await findSentenceList(req.params.studentid,req.params.taskid);
      res.render("task-performance",{sentencelist:sentencelist,taskid:req.params.taskid});
    }
    catch(err)
    {
    ...
    User.find({},function(err,users)
    {
      if(err)
      {
        ...
        users.forEach(function(user) {
          studentlist.push({id:user._id, username:user.username, fname:user.fname, lname:user.lname, class:user.class, dob:user.dob});
        });
      ...
      User.findById(userid,function(err, user){
        if(err){
          ...
          try{
            ...
            {
              tasklist_0_1.push([user.tasks[i].sentenceList.length, user.tasks[i].status]);
            }
  ...
  if(req.isAuthenticated())
  {
    User.findById(req.user._id,function(err, user){
      if(err){
        ...
        res.render("profile",{username: user.username, fname:user.fname, lname:user.lname, dob:user.dob, _class:user.class});
      }
    });
  ...
  if(req.isAuthenticated())
  {
    User.findById(req.user._id,function(err, user){
      if(err){
        ...
        res.render("admin-profile",{username: user.username, fname:user.fname, lname:user.lname, dob:user.dob});
      }
    });
...
app.post("/profile", async function(req,res){
  if(req.isAuthenticated())
  {
    User.findByIdAndUpdate(req.user._id, {fname:req.body.fname, lname:req.body.lname, dob:req.body.dob, class:req.body.class}, function(err, user){
      if(err){
      ...
      .then(user => {
          user.changePassword(req.body.oldpassword, req.body.newpassword)
              .then(() => {
                  ...
                  res.render("profile",{username: user.username, fname:user.fname, lname:user.lname, dob:user.dob, _class:user.class, success:"Changed password sucessfully!"});
              })
              .catch((err) => {
                console.log("At post /changePassword 1 \n"+ err);
                res.render("changePassword",{err:"Existing password is incorrect!"});
              })
      })
      .catch((err) => {
...
app.get("/performance/:taskid", async function(req,res){
  ...
  {
    try{
      const sentencelist = await findSentenceList(req.user._id,req.params.taskid);
      res.render("task-performance",{sentencelist:sentencelist,taskid:req.params.taskid});
    }
    catch(err)
    {
  ...
  if(req.isAuthenticated()){
    ...
    try{
      ...
      if(sentid>=_user.tasks[taskid].sentenceList.length)
      {
        ...
        await User.findByIdAndUpdate(req.user._id, {tasks: _user.tasks}, function(err, user){
          if(err){
            console.log(err);
            throw err;
          }
...
app.post("/task/:taskid/:sentid/0/next", async function(req, res)
{
  if(req.isAuthenticated()){
    ...
    try{
      ...
      await User.findByIdAndUpdate(req.user._id, {tasks: user_tasks}, function(err, user){
        if(err){
          console.log(err);
          throw err;
        }
        else{
          console.log(user.tasks[taskid].sentenceList[sentid].words[0].finishTime);
          res.render("task", {
            text : sentence,
            taskid: taskid,
            sentid: sentid,
  ...
  if(req.isAuthenticated())
  {
    ...
    var p1 = new Promise(async function(resolve,reject){
      try{
        ...
        word = user_tasks[taskid].sentenceList[sentid].words[parseInt(wordno)].word;
        prevTime = user_tasks[taskid].sentenceList[sentid].words[parseInt(wordno)-1].finishTime;
      ...
      //if(inputWord==word)
      {
        ...
        user_tasks[taskid].sentenceList[sentid].words[parseInt(wordno)].finishTime = await newTime;
        user_tasks[taskid].sentenceList[sentid].words[parseInt(wordno)].timeTaken = interval;
        //console.log(5+" "+inputWord+" "+word);
      }
    }).then(function() {
      {
        User.findByIdAndUpdate(req.user._id, {tasks: user_tasks}, function(err, user){
          if(err)
          {
            reject(err);
          }
          ...
          //console.log(6);
          console.log("last word: "+word+" = "+user.tasks[taskid].sentenceList[sentid].words[parseInt(wordno)].timeTaken);
  ...
  if(req.isAuthenticated())
  {
    ...
    try{
      ...
      if(sentence==(req.body.sentence+' '))
      {
        ...
        var p1=new Promise(function(resolve,reject){
          _user.tasks[taskid].sentenceList[sentid].inputSentence=req.body.sentence;
          resolve();
          //return _user.tasks;
        }).then(function(){
          console.log(_user.tasks[taskid].sentenceList[sentid].inputSentence);
          User.findByIdAndUpdate(req.user._id, {tasks: _user.tasks}, function(err, user){
            if(err){
              console.log(err);
              throw err;
            }
